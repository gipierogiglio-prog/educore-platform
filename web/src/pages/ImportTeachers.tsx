...
export function ImportTeachers() {
  ...
  const handleFile = async (f: File) => {
    ...
    try {
      const data = await api.post<PreviewResponse>('/api/teachers/import/preview', formData)
      setPreview(data)
    } catch (err: any) {
  ...
  const handleImport = async () => {
    ...
    try {
      const data = await api.post<ImportResult>('/api/teachers/import/execute', formData)
      setResult(data)
    } catch (err: any) {
      setError(err.message)
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Importar Professores</h1>
        <p className="text-muted-foreground">
          Faça upload de um arquivo CSV ou Excel com os dados dos professores
        </p>
      </div>
        ...
        <CardContent>
          {!file ? (
            ...
            <div className="flex items-center justify-between p-3 bg-secondary rounded-lg">
              ...
              <Button variant="ghost" size="icon" onClick={reset}><X size={16} /></Button>
            </div>
          )}
        </CardContent>
      </Card>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left p-2 font-medium">Especialização</th>
